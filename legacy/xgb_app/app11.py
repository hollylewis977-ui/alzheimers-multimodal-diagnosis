...
@app.route('/')
def index():
    # 与第二份一致：构造前端元信息 input information
    meta = {}
...
def predict():
    ...
    data = request.get_json(silent=True) or request.form.to_dict()#输入
    row = {}
    for f in feature_order:
        try:
    ...
    df_t = transform_features(df, scaler)#transform featrues
...
def download_template():
    """Generate and download a CSV template with all required columns."""
    # Create a template DataFrame with one sample row
    template_data = {
        'PatientID': ['SAMPLE001'],
        'Age': [70],
        'Gender': [0],  # 0=Male, 1=Female
        'Ethnicity': [0],  # 0=White, 1=African American, 2=Asian, 3=Other
        'EducationLevel': [2],  # 0=None, 1=Primary, 2=Secondary, 3=Tertiary
        'BMI': [25.5],
    ...
    # Create a BytesIO buffer to save the CSV
    buf = io.BytesIO()
