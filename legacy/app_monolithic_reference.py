...
# ==== 2D MRI via PyTorch ResNet (binary) ====
MRI_RESNET2D_PATH = os.path.join(ART_DIR, "best_model.pth")
mri_model_resnet2d = None
torch_2d_transform = None
...
try:
    ...
    if os.path.exists(MRI_RESNET2D_PATH):
        ckpt = torch.load(MRI_RESNET2D_PATH, map_location="cpu")
        ...
        print("[MRI] 2D ResNet model loaded:", MRI_RESNET2D_PATH)
    else:
        print("[MRI] 2D ResNet weight not found:", MRI_RESNET2D_PATH)

except Exception as e:
    print("[MRI] 2D ResNet init failed:", repr(e))
    mri_model_resnet2d = None


def _predict_img_bytes_torch(img_bytes) -> Optional[float]:
    """用 2D-Torch-ResNet 对单张图片得到 AD 概率"""
    ...
    with torch.no_grad():
        logit = mri_model_resnet2d(x)        # (1,)
        prob = torch.sigmoid(logit).item()   # 转成概率
    return float(prob)
    ...
    if file_mri and file_mri.filename:
        ...
        try:
            if fname.endswith('.zip'):
                ...
                raw = file_mri.read()
                file_mri.stream.seek(0)
                p_mri = _predict_img_bytes_torch(raw)
                if p_mri is None:
                    mri_msg = '2D ResNet model not loaded'
            else:
